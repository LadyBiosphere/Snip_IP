...
$(document).ready(function() {
        // DOM is fully loaded,
        // but images and other resources maybe not.
        $('.nav-toggle').click(function(evt) {
                // .nav-toggle was clicked
                $('#background-container nav ul').stop().slideToggle('slow');
                evt.preventDefault();
                return false;
        });
        $(window).resize(function() {
                $('background-container nav ul').stop().hide();
                if ($(window).width() >= 640) {
                        $('background-container nav ul').show();
                }
        });
});
